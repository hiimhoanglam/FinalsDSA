    ...
    public Collection<Move> getLegalMoves(final Board board) {
        ...
        for (int candidateOffsets: CANDIDATE_OFFSETS) {
            ...
            if (BoardUtils.isValidCoordinate(candidatePosition)) {
                 ...
                 */
                if (candidateOffsets == 16 && this.firstMoveCheck.isFirstMove() && !board.getTile(candidatePosition).isOccupied()
                    &&  (secondRowExclusions(candidatePosition,candidateOffsets) && this.pieceAlliance == Alliance.BLACK)
                    ||  (seventhRowExclusions(candidatePosition,candidateOffsets) && this.pieceAlliance == Alliance.WHITE)){
                    int betweenDestination = this.piecePosition + (this.pieceAlliance.getValue() * 8);
                    if (!board.getTile(betweenDestination).isOccupied()) {
                        legalMoves.add(new Move.MajorMove(board,this,candidatePosition));
                    }
                }
                /*
